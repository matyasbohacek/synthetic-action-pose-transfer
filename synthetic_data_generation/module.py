...
class GaussianRenderer(nn.Module):
    ...
    def forward(self, gaussian_assets, img_shape, cam_param, bg_image_path=None):
        ...
        if bg_image_path is not None:
            if isinstance(bg_image_path, torch.Tensor):
                bg_image_path is actually a Tensor (an image), so use it directly
                bg_image = bg_image_path
            else:
                bg_image_path is a file path, so open the image from the file
                ...
                bg_image = bg_transform(bg_image)
                #print(f"highest_value:{bg_image.max()}")
                #print("bg_image shape:", bg_image.shape)
            bg_image.cpu()
        else:
            bg = torch.ones((3, img_shape[0], img_shape[1])).float().cuda()
        ...
        render_alpha = torch.clamp(render_mask, 0, 1)

        if render_alpha.dim() == 2:
            render_alpha = render_alpha.unsqueeze(0)

        render_img = render_img.cpu()
        render_alpha = render_alpha.cpu()

        render_img = render_img * render_alpha + bg_image * (1 - render_alpha)
